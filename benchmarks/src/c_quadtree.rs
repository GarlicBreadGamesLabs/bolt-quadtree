...
pub fn run(entities: usize, ticks: usize) -> Result<CQuadMetrics, String> {
    let repo_dir = ensure_repo()?;
    ensure_submodules(&repo_dir)?;
    let binary = build_headless(&repo_dir, entities, ticks)?;
...
fn build_headless(repo_dir: &Path, entities: usize, ticks: usize) -> Result<PathBuf, String> {
    ...
    let mut args = vec![
        ...
        "-DNDEBUG".to_string(),
        "-DHEADLESS".to_string(),
        format!("-DHEADLESS_TICKS={}", ticks),
        format!("-DITER={}", entities),
        format!("-DMEASURE_TICKS={}", ticks),
        "-D_GNU_SOURCE".to_string(),
        "-DALLOC_DEBUG".to_string(),
        "-Wall".to_string(),
    ...
    cmd.args(["-include", "stdbool.h"]).arg("-lm");
