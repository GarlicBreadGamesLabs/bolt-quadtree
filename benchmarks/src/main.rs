...
const QUADTREE_F32_MAX_ENTITIES: usize = 1_000;
...
fn bench_quadtree_f32(entities_seed: &[Entity], bounds: Bounds, ticks: usize) -> BenchResult {
    ...
    BenchResult {
        ...
        notes: if capped {
            "relocate = remove+insert per entity; capped at 1k to avoid stack overflow"
        } else {
...
fn main() {
    ...
    if should_run(filter.as_deref(), "c-quadtree") {
        match c_quadtree::run(entity_count, ticks) {
            Ok(metrics) => {
