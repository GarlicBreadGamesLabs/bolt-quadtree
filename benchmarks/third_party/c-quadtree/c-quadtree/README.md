...
- Added a `HEADLESS` build path in `window.c` with a configurable tick budget for the benchmark harness.
- Bench harness compiles with `-DHEADLESS`, `-DHEADLESS_TICKS`/`-DMEASURE_TICKS`, and forces `stdbool.h` include.
